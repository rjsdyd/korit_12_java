...
/*
    별 찍기 관련 부분을 저희가 메서드화 시켜볼 예정입니다.
 */
public class Method06StarWriting {
    // call4() 유형으로 작성할 예정 -> 몇 줄 짜리 별 찍을 건지
    // 어떤 유형의 별 찍기를 할건지 main에서 받을 예정이기 때문에.
    // method 정의 영역
    public static String getStar(int row, int option) {
        // 메서드 내에서만 사용되는 지역 변수 선언 및 초기화
        String result = "";
        // 별이 어떻게 찍히게 될지와 관련된 로직을 작성해야합니다.
        if (option < 1 || option > 4) {
            result = "불가능한 숫자 입력입니다. 1 ~ 4의 정수만 입력해주세요.";
        } else {        // 이 부분이 실행된다면 어짜피 .option은 1, 2, 3, 4중에 하나니까
            if (option == 1) {
                // 공백을 책임지는 바깥쪽 for
