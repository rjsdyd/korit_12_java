...
//        System.out.println(name);
        /*
            ...
            2. next (Int / double / Boolean, ... etc) vs nextLine
                ...
                - 나머지는 허용이 안됩니다. 그래서 kim 2로 하니까 kim만 저장됐습니다. 그리고 kim2로 바꿔 저장했을 때만 가능했습니다.
                - nextLine()이 엔터 키를 기준으로 데이터가 들어가는 점은 어떤 부분에서 사람을 귀찮게 하는 부분이 있습니다.
         */
...
//        System.out.println("이름 : " + name2);
        ...
        /*
            이상의 코드에서의 문제점은 45번 라인이 실행됐을 때 데이터를 입력 받지 않고
            그래로 넘어가서 46번 라인과 47번 라인이 실행된다는 점입니다.
            이상의 문제는 nextInt()에서 값을 입력받고 enter를 치게 되는데,
            그 경우 다음 nextLine()에서 enter키를 받아들여서 빈 값인 상태로 enter를
            쳤다고 알아듣기 때문에 곧장 46번 라인이 실행됐습니다.
        ...
        scanner.nextLine();
        // 얘는 enter만 받아주는거지 그 데이터를 어디다 쓸게 아니니까 변수에 저장할 필요는 없겠죠.
        // 변수에 저장한다는 건 메모리를 할당한다는 의미니까요.
