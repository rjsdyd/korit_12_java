...
/*
    배열의 출력
    엄밀히 말하면 저희는 배열을 출력한게 아니라 배열 내부의 element들을 출력해 왔습니다.
    System.out.println(arr01);과 같은 출력문을 작성했을 때,
    주소값이 튀어나온다는 사실을 확인할 수 있습니다.
    ...
    이상에서 확인해야 할 부분은 동일한 method명이라고 하더라도
...
//        System.out.println(Arrays.asList(numbers).toString());
        ...
        /*
            이상의 경우에는 sort() 메서드에 오름차순에서는 argument가 하나 있는데,
            내림차순할 때는 보니까 두 개 입니다.
            이상의 과정이 overloading이 이루졌다는 점을 파악할 수 있으면 현재 수준으로는
            만족스럽습니다.

            그리고 두 번째 argument로 Comparator.reverseOrder()가 사용되었는데,
            메서드의 결과값이 sort()의 두 번째 argument로 사용되었다는 점에서
            함수형 프로그래밍의 일종이라고 볼 수 있습니다.
            근데 기본 자료형 (primitive types)인 int[] 배열을 사용할 수 없어서
            Integer[] 배열을 사용했다는 점도 특이사항 중 하나인데,
            현재 수준으로는
