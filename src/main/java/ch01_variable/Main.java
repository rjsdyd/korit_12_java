...
/*
    ...
    즉, 주석이란 컴퓨터가 읽어서 처리하는 부분이 아니라 사람이 읽어서 정보를 얻을 수 있는 것에 의의가 있습니다.
    1) 한 줄 주석 : //
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("1");
        /*
            이상과 같은 방식의 코드 대입 (매번 데이터를 직접 사용하는 방식) 방식을 '하드 코딩'이라고 합니다.
            하드 코딩 : 명령문에 데이터 (변수에 대입되지 않은 데이터인데 이를 리터럴 (literal))를 바로 집어넣는 것으로,
                      '지양'되는 방식.

            그렇다면 권장되는 방식은 ? 데이터를 '변수'에 대입하고, 변수를 통해서 명렬을 내리는 방식
        ...
        */
        ...
        String introduction = "안녕하세요, 저는 이번 국비 과정을 참여하게 된 이건용입니다.";
        ...
        /*
            Java에서의 변수 명명 규칙 :
                ...
                cf) python 학습 하신 분들의 경우 python 변수 표기 방식은
                    스네이크 표기법 (snake case)라고 해서 my_test_result와 같은 방식으로 작성합니다.
         */
        // 변수 : 값이 바뀔 수 있음.
        System.out.println("국어 점수(변경 전) : " + scoreKorean);
        scoreKorean = 100;          // 재대입할 때는 자료형을 명시하지 않는다.
        System.out.println("국어 점수(변경 후) : " + scoreKorean);

        // 자료형 수업
        // 1. 논리 자료형(boolean) 변수 : 참 / 거짓 (true / fales)
        // 자료형 변수명 = 데이터(값);
        ...
        // 근데 70 라인의 경우 변수의 자료형이 char인데 문자가 여러 개 나오는 바람에 결과값이 의도한 대로 나오지 않음.
        ...
        /*
            ...
            개발자인 저희 하나하나 지정을 해줄 필요가 있습니다.
            '문자'와 '문자열'은 서로 다른 개념인데,
            문자 (하나짜리)를 세 번 더했을 경우 더 이상 문자가 아니라 문자열로 인식되기 때문에
            원하는 방식으로 출력되지 않습니다.

            그렇기 때문에 sout() 내부에 ""를 넣어주니까, 컴퓨터는 얘가 문자열이라고 생각하고 원하는 결과값을 만들어줬습니다.

            즉
            작은 따옴표 ('')는 '문자'를 표시하기 위한 기호
            큰 따옴표 ("")는 "문자열"을 표기하기 위한 기호라고 알아두면 되겠습니다.
            ...
            과제 : name1, name2, name3에 여러분들의 이름을 한 글자씩 각각 대입하여
            ctrl + f10을 눌렀을 경우에
            김일이
            라고 출력될 수 있도록 작성하시오.
            그리고 한 줄 더 추가하시오.
        ...
        /* my 풀이
            ...
            System.out.println("안녕하세요 제 이름은 " + myname1 + myname2 + myname3 + "입니다.");
        */
        // 강사님 풀이
        name1 = '이';
        name2 = '건';
        ...
        // 자료형 int 뿐만 아니라 대문자로 시작되는 자료형인 Integer가 있긴 함 (추후 학습)
