...
public class AnimalData<T> {
    ...
    /*
        ...
        Tiger / Human 객체를 field에 대입할 것임

        근데 T에는 어떠한 클래스도 들어갈 수 없기 때문에 현재 상황에서는
        Car 클래스의 객체도 animal에 들어갈 수 있다고 해석할 수 있음

        이번 목표
        특정 클래스라면 (즉 Animal 클래스의 상속을 받은 서브클래스라면)
        해당 클래스에 맞게 객체 정보를 출력하도록 작성할 것임.
     */
    public void showData() {
        ((Animal) animal).move();       // 지금 보면 T에는 아무 클래스나 들어갈 수 있으므로
                                        // (Animal) 부분은 Animal로 형변환해주겠음.
        if (animal.getClass() == Human.class) {
            ((Human) animal).read();    // 현재 필드에 들어간 animal
