...
public class FactoryMain {
    public static void main(String[] args) {
        ...
        /*
            ...
            재사용하지 않고 한 번만, 즉 이 경우에는 FactoryMain에서만 한 번 쓰고 치운다면 사용하기 좋습니다.

            저희는 교육 상황이기 때문에 모든 클래스를 한 번 정의하고 그걸로 객체 한 두 개 만들고 치우는 경우가
            많아서 모든 수업을 위처럼 익명 클래스로 정의할 수도 있지만
            기본적으로 여러 프로그램에서 동시에 쓸 객체가 아닌 경우에 익명 클래스를 통해
            임시 객체 생성이 가능하도록 지원한다고 보시면 됩니다.
         */
        // PhoneFactory 객체 생성
        ...
        /*
            어제 상속 배운 부분과 합쳐서 생각을 해볼 때,
            추상 클래스는 상속을 전제로 한다는 것을 알 수 있습니다.
            하지만 Java 버전 업에 따라 익명 클래스를 통해 FactoryMain에서만
            임시적으로 쓸 수 있는 객체를 생성하는 것도 가능합니다.

            그리고 메서드 오버라이딩의 경우 '재정의'를 전제로 하고 있기 때문에
            부모 클래스와 자식 클래스의 메서드 호출 결과가 '다르게' 굴러갈 경우에만
            @Override가 요구되고, 동일한 경우에는 그냥 부모 클래스의 메서드를
            호출하면 되죠.

            그러면 개발자인 우리는 부모 클래스에만 적힌 메서드의 호출 방식(call1() ~ 4())과
            return에 대해서 명확하게 알고 있어야지만 자식 클래스에서
